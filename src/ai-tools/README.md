...
Installs Claude Code, OpenAI Codex, and Happy globally. Mounts persistent config volumes for each tool and bind-mounts the shared agents/skills directory from the host. Persists ~/.claude.json (onboarding state, project trust, MCP config) by symlinking into the .claude volume.
